...
**认知模式（v1.3+）**：通过 Domain Engineer 角色，帮助团队对重要的既有模块形成系统性的领域知识（Domain Knowledge）——AI 读取源码，提炼业务实体、流程全景、接口与集成、设计决策，资深人员引导生成，沉淀为团队认知资产。v1.6.0 新增 Coach 角色（**协作认知**），在 `/ilink-approve` 时评估人类在 PM/Design 修订段的输入质量与对 design 文档的直接编辑，给出可执行的协作改进建议。
...
> **注意**：`/ilink-refine` 是修订对话，仅更新文档内容，完成后文档仍保持 STAGING 状态；`/ilink-approve` 才是正式的状态推进动作，且仅接受当前 Status=STAGING 的文档。从 v1.6.0 起，`/ilink-approve` 为 Slash Command（不再是 shell 脚本），内部串行执行 Coach 子流程后再完成 Status 推进，详见 Root Spec §4.6 与 §6.4。

#### 认知工作线（资深引导）

认知工作线**独立于交付流水线**，不绑定工单，由资深工程师判断哪些模块值得沉淀。当前形态为 Domain Knowledge（业务领域沉淀）：

```
资深工程师选定模块 → /ilink-domain <module-id>
    ↓
AI 读取源码 → 提炼业务实体、流程全景、接口与集成、设计决策
    ↓
...
| **认知与交付分离** | 领域知识沉淀、Coach 协作复盘（认知模式）与工单开发（交付模式）是独立工作线，不互相阻塞，各有其触发条件和产出形式 |
...
**步骤 2 — Command 文件检查**：
- 校验当前平台的 Command 文件（如 `.claude/commands/ilink-{pm,design,coder,qa,init,refine,approve,status,domain,pull}.md`）
