...
**认知模式（v1.3+）**：通过 Domain Engineer 角色，帮助团队对重要的既有模块形成系统性的领域知识（Domain Knowledge）——AI 读取源码，提炼业务实体、流程全景、设计决策、见贤思齐，资深人员引导生成，沉淀为团队认知资产。v1.4.11 新增 SDD Assessment Engineer 角色，用于评估模块是否适合 SDD 实践；v1.6.0 新增 Coach 角色（**协作认知**），在 `/ilink-approve` 时评估人类在 PM/Design 修订段的输入质量与对 design 文档的直接编辑，给出可执行的协作改进建议。
...
/ilink-pm（理解需求 → 输出业务合同）
    ↓
├─ STAGING → /ilink-refine（修订文档，仍保持 STAGING）→ /ilink-approve（含 Coach 子流程）正式推进
└─ PENDING_DESIGNER →
/ilink-design（技术设计 → 输出设计方案 + 文件级任务清单 + 写入 design 快照）
    ↓
├─ STAGING → /ilink-refine（修订文档，仍保持 STAGING）→ /ilink-approve（含 Coach 子流程）正式推进
...
> **注意**：`/ilink-refine` 是修订对话，仅更新文档内容，完成后文档仍保持 STAGING 状态；`/ilink-approve` 才是正式的状态推进动作，且仅接受当前 Status=STAGING 的文档。从 v1.6.0 起，`/ilink-approve` 为 Slash Command（不再是 shell 脚本），内部串行执行 Coach 子流程后再完成 Status 推进，详见 Root Spec §4.7 与 §6.4。
...
| **认知与交付分离** | 领域知识沉淀、SDD 评估、Coach 协作复盘（认知模式）与工单开发（交付模式）是独立工作线，不互相阻塞，各有其触发条件和产出形式 |
...
| `ilink-approve <story>` | Slash Command（v1.6.0 起）：调用 Coach 子流程 → 写入 `<story>-feedback.md` → 将 STAGING 推进到下一个状态 |
| `ilink-pull <story>` | 从 project-context.md 配置的 Issue System 拉取需求"描述"字段，写入 requirement.md 的"## 功能描述"区块；bash 实现位于各平台 commands 目录下 |
...
**步骤 1 — 框架文件检查**：
- 校验 `iLink/iLink-root-spec.md` 与 `iLink/souls/{universal,pm,design,coder,qa,domain,sdd,coach}.soul.md` 是否存在
- 全部缺失时 MUST 停止执行，提示用户先复制 iLink 框架到项目

**步骤 2 — Command 文件检查**：
- 校验当前平台的 Command 文件（如 `.claude/commands/ilink-{pm,design,coder,qa,init,refine,approve,status,domain,sdd,pull}.md`）
- Claude 平台还需校验 `.claude/commands/ilink-pull.sh`（bash 脚本本体）
- 缺失时 SHOULD 给出警告但不阻塞（用户可能使用其他平台；`ilink-pull.*` 缺失时只影响 Issue System 拉取功能，不影响主流水线）
...
> **注意**：文件顶部除了下述通用 AI 行为规则块外，bootstrap MUST 紧接其后追加 **Issue System 集成 AI 隔离块**（详见本节末尾"Issue System 集成块"小节）。该块定义 `/ilink-pull` 所需的外部接口配置，受 Root Spec §7.8"AI 隔离块识别"约束，AI Agent 不读取、不修改其内容。
...
**Issue System 集成块**（bootstrap MUST 紧接通用 AI 行为规则块后追加）：
...
**写入策略**（幂等）：
- 若 `project-context.md` 中已存在以 "Issue System 集成" 结尾标题的块 → 跳过（保留用户已修改的 project_name）
- 若不存在 → 紧接通用 AI 行为规则块（即包含"最后更新：YYYY-MM-DD"行的那一块）之后追加完整块
- **bootstrap 不询问用户**——无论用户是否计划使用 `/ilink-pull`，都默认写入；不想用的用户保留 `<待填写>` 即可（`/ilink-pull` 会礼貌拒绝并指引编辑位置）
...
本项目使用 iLink 多角色 AI 协作流水线（Root Spec v1.6.0）。
...
/ilink-approve <story>  → Human-Gate 推进：调用 Coach 子流程（写入 <story>-feedback.md）→ STAGING → 下一状态
...
/ilink-bootstrap              — AI 分析项目 + 更新入口路由（一次性）
    ↓
/ilink-init <story> <usage-value>   — 每个 Story 都需要，第二参数必填（v1.6.0）
    ↓
/ilink-pm → /ilink-design → ...（正常流水线）
    ↓
/ilink-qa <story> <usage-value>     — 第二参数必填（v1.6.0）
```

> **v1.6.0 起 `/ilink-init` 与 `/ilink-qa` 的"硬接口"**：第二参数 `<usage-value>` 必填，缺失则命令拒绝执行。用户在执行命令前先在本平台查询"已使用"指标（Qoder `/usage`、Claude `/usage`、Codex `/status`、Gemini `/stats`），将数值传入。无法查询时允许传 `0` 占位。详细协议见 Root Spec §8.1.2。中间阶段 PM/Designer/Coder 命令签名**完全不变**，零侵入。
