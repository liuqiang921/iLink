...
`project-context.md` 中可能存在**AI 隔离块**——以 `> **AI 行为规则（CRITICAL，所有 AI 必须遵守）— <子主题>**` 开头的 blockquote 块（标题尾部带 "— <子主题>" 后缀，与文件最顶部的通用 AI 行为规则块明显区分）。该类块承载 iLink bash 脚本的工具配置（如 Issue System 的 URL、项目名），**不是项目知识**。

读取 `project-context.md` 时：

- **MUST 识别并跳过**所有 AI 隔离块——其内的 `api_url`、`project_name` 等配置 SHALL NOT 作为业务理解上下文、需求假设、技术决策依据
- **MUST NOT 修改**隔离块内容——用户请求修改时，提示其手动编辑或重跑 `/ilink-bootstrap`
- 文件最顶部不带 "— <子主题>" 后缀的"AI 行为规则"块是通用规则（关于本文件本身的读写策略），属于必读内容，**不是**隔离块
