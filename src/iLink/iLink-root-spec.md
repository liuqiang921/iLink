...
2. [架构](#2-架构)
3. [流水线协议](#3-流水线协议)
4. [角色行为规范](#4-角色行为规范)（含 4.5 Domain Engineer、4.6 Coach）
...
Step 2  AI 提取接口与集成（§4）
        → grep @BexMethod / @Autowired / @DataSource / @Value
        → 把对外功能号、内部依赖、数据库依赖、配置依赖整理成四张表
        → 每行对应可定位的代码（类.方法 / xml 路径 / 配置 key）

Step 3  AI 生成 [待确认] 清单
        → 标出代码里看不出来的"为什么"
        → 人类带着清单找业务专家逐条确认

Step 4  业务专家回答问题（获取输入——补充 AI 从代码读不到的"为什么"）
        → 人类将业务专家的回答反馈给 AI
...
| §4 接口与集成 | 对外功能号 / 内部依赖 / 数据库依赖 / 配置依赖（grep 机械提取） | AI从代码读取 |
| §5 内部机制 | 复杂子系统的内部结构（简单模块可省略） | AI从代码读取 |
| §6 业务规则 | 关键业务决策及其原因 | AI推导 + 人工补充 |
| §7 设计决策 | 技术选型和架构决策的原因 | AI推导 + 人工补充 |
| §8 配置参数 | 影响生产行为的关键开关速查表 | AI从代码读取 |
| §9 故障模式 | 已知故障场景、内置机制、处理预案 | AI推导 + 人工补充 |
...
**定位**：Coach **不是**流水线角色，也**不是**业务认知角色（Domain），而是**协作认知角色**——评估的对象是"人类如何与 AI 协作"，沉淀的是团队对协作方法的反思。
...
### 4.6.9 快照机制（design.master.md 直接编辑检测）
...
2. 调用 Coach 子流程（见 §4.6）→ 写入 <story>-feedback.md
