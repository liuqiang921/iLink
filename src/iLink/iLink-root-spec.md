...
2. [架构](#2-架构)
3. [流水线协议](#3-流水线协议)
4. [角色行为规范](#4-角色行为规范)（含 4.5 Domain Engineer、4.6 SDD Assessment Engineer、4.7 Coach）
5. [Metadata 协议](#5-metadata-协议)
6. [Human-Gate 协议](#6-human-gate-协议)
7. [Agent 通用行为协议](#7-agent-通用行为协议)
8. [文件系统与目录结构](#8-文件系统与目录结构)（含 8.1.2 Usage 追踪协议）
9. [安全与执行保障](#9-安全与执行保障)
10. [版本控制集成](#10-版本控制集成)
11. [约束与限制](#11-约束与限制)
...
| Issue System | 类 Jira 的外部需求/工单系统。`/ilink-pull` 通过 HTTP 调用其接口按 story-id 拉取需求描述，写入本地 `<story>-requirement.md` |
| AI 隔离块 | `project-context.md` 中以"AI 行为规则（CRITICAL，所有 AI 必须遵守）— <子主题>"形式开头的 blockquote 块。该类块的配置项仅供 iLink bash 脚本读取，AI Agent MUST NOT 将其内容作为业务理解上下文，MUST NOT 修改其内容 |
...
>
> | 平台 | AI 命令（pm / design / coder / qa / bootstrap） | 工具命令（init / approve / status / pull） |
> |------|------------------------------------------------|-----------------------------------|
> | **Claude CLI** | Slash Command（`.claude/commands/*.md`），Host CLI 原生读取执行 | 同左，全部为 Slash Command（`/ilink-pull` 为 .md 薄壳调用同目录 .sh） |
...
6. **认知与交付分离**：认知线活动（Domain Knowledge、SDD Assessment、Coach 协作复盘）与 Story 开发流程（交付线）MUST 保持独立，SHALL NOT 将 `ilink-domain` 或 `ilink-sdd` 纳入 PM→QA 流水线序列；Coach 虽嵌入 `/ilink-approve` 触发，但其产出（feedback.md）SHALL NOT 进入流水线契约链，仅供人类复盘。认知线产出是交付线的**输入参考、元决策依据或团队能力提升资产**，而非前置节点；其中 Domain Knowledge 沉淀团队对**业务领域**的理解，SDD Assessment 沉淀团队对**模块是否适合 SDD**的元决策判断，Coach 沉淀团队对**与 AI 协作方法**的反思。
...
**职责**：作为协作复盘伙伴，在 `/ilink-approve` 触发时评估**人类在 PM 与 Design 修订段的输入质量**和**对 design.master.md 的直接编辑**，给出可执行的改进建议，写入 `<story>-feedback.md`。

**定位**：Coach **不是**流水线角色，也**不是**业务认知角色（Domain）或方法论评估角色（SDD），而是**协作认知角色**——评估的对象是"人类如何与 AI 协作"，沉淀的是团队对协作方法的反思。

**SHALL NOT**：
- 评估 AI 自身的表现（已被各角色 Soul 约束）
- 评估输出物（master doc）的功能质量（QA 的职责）
- 进入流水线契约链（feedback.md SHALL NOT 被下游角色消费）
- 影响文件状态机（SHALL NOT 改变 Status）
- 携带 Metadata 印章（feedback.md 不属于 master 体系）

### 4.7.1 触发条件

Coach MUST 在以下场景触发：
- 人类在 STAGING 状态执行 `/ilink-approve <story>` 时，由 `/ilink-approve` 命令内部串行调用 Coach 子流程
- Coach 评估完成后，再由 `/ilink-approve` 命令完成 Status 状态推进

Coach SHALL NOT 被独立调用——它是 `/ilink-approve` 的子流程，不存在 `/ilink-coach` 单独命令。

### 4.7.2 输入

| 输入 | 说明 |
|------|------|
| 当前 Host CLI 会话的对话原文 | Parent AI 在主对话中持有；逐 turn 摘录后传入 subagent |
| `<story>-design.master.md` 当前内容 | 用于直接编辑差异检测 |
| 最近一次 design 快照（`iLink-doc/<story>/.snapshots/design.master.<timestamp>.md`） | 由 `/ilink-design` 完成时落盘；与当前 design.master.md 做 diff |
| `coach.soul.md` | Coach 角色规范（评估维度 + 输出标准） |

**Coach SHALL NOT 读取**：pm.master.md、requirement.md、源码文件、`<story>-feedback.md` 历史轮次。
...
```
Step 1  Parent AI 识别对话 bracket
        外层窗口：/ilink-pm <story> 完成之后的第一个人类 turn
                  → /ilink-approve <story> 调用之前的最后一个人类 turn
        内部分界：外层内的 /ilink-design <story> slash 调用 turn

Step 2  Parent AI 逐 turn 原文摘录
        - 标注 [turn-N] 与角色（user/assistant）
        - 标注内部分界 turn 为 [--- /ilink-design 分界 ---]
        - SHALL NOT 改写、概括或选择性剔除

Step 3  Parent AI 计算直接编辑 diff
        - 对比 design.master.md 当前内容与最近一次 .snapshots/ 快照
        - 输出 unified diff 文本
        - 若快照缺失，记录"无法判定直接编辑"

Step 4  Parent AI 生成 subagent 调用
        - 传入：对话摘录 + 直接编辑 diff + coach.soul.md
        - SHALL NOT 传入 master doc、requirement.md、源码

Step 5  Subagent（fresh context）评估
        - 自主判定 turn 噪声
        - 按 6 维度评估对话修订段
        - 按 4 维度评估直接编辑段
        - 按 4 输出标准格式化反馈

Step 6  Parent AI 写入 feedback.md
        - 文件路径：iLink-doc/<story>/<story>-feedback.md
        - 追加新一轮反馈（带时间戳），不覆盖
        - 然后将控制权交回 /ilink-approve 主流程
```

**为什么使用 subagent 而非 parent 自评**：parent AI 是对话的参与者，自评存在维护偏见；subagent 没参与过这段对话，等于"独立观察者审参与者"，等价于 PM→Designer→Coder→QA 链式审查的"下游审上游"原理。
...
| +5 | 抽象层（复用对话维度） | diff 内容是否在错的层级 | 在 §2 系统逻辑分析里塞了类设计 |
| +6 | 范围纪律（复用对话维度） | diff 是否引入跨阶段内容 | 在 design 文档里加了 PM 才该写的需求 |
...
**命名规则**：
- 文件名 SHALL NOT 携带 `master` 后缀——它不是 master 文档，不进协议契约链
...
| 段内有建议 | 列出 ≤3 条 |
| 段内被评估但无问题 | "本段无建议" |
| 段内无修订 turn / 无直接编辑 | "本段无修订（一次通过）" / "本段无直接编辑" |

每个阶段下"对话修订"和"直接编辑"两个子节 MUST 都出现，对称结构便于回顾趋势。

### 4.7.8 与交付流水线的关系

- Coach **不产生** STAGING / PENDING 等流水线状态，**不影响** `/ilink-approve` 的状态推进结果
- `/ilink-approve` 的执行顺序：校验前置 → 调用 Coach → 写入 feedback.md → 推进 Status
- 即便 Coach 子流程失败（如 subagent 调用异常），SHALL NOT 阻塞 `/ilink-approve` 的状态推进；MUST 在 feedback.md 中记录错误原因
- feedback.md SHALL NOT 被任何下游 AI 角色（Coder / QA）作为输入读取
- feedback.md MUST 提交到 git（与 master doc 一同管理），便于团队复盘趋势

### 4.7.9 快照机制（design.master.md 直接编辑检测）

- `/ilink-design <story>` 完成时，MUST 把刚写入的 `design.master.md` 复制一份到 `iLink-doc/<story>/.snapshots/design.master.<timestamp>.md`
- 时间戳格式：`YYYYMMDD-HHMMSS`，使用 `TZ=Asia/Shanghai date +%Y%m%d-%H%M%S` 获取
- `/ilink-approve <story>` 触发 Coach 时，对比 `design.master.md` 当前内容与 `.snapshots/` 中**最新**的快照（按文件名排序取最大）
- `.snapshots/` 目录 MUST 加入 `.gitignore`，SHALL NOT 提交到 git
- 快照机制不依赖 git，跨 CLI 通用
...
/ilink-approve <story>
  ↓
1. 校验前置：当前 STAGING 文档存在且 Status = STAGING
2. 调用 Coach 子流程（见 §4.7）→ 写入 <story>-feedback.md
3. 完成 Status 状态推进 → 对应的 PENDING_* 状态
4. 报告：feedback 文件路径 + 状态变更结果
```
...
- MUST 检查当前 Status 是否为 STAGING，非 STAGING 时打印提示并退出（含幂等性：已是 PENDING_* 时跳过 Coach 与状态推进）
- Coach 子流程仅在 design.master.md 推进场景启用（pm.master.md 与 review.master.md 推进场景跳过 Coach，但 MUST 保留 Status 推进逻辑）
- 即便 Coach 子流程失败（subagent 调用异常等），SHALL NOT 阻塞 Status 推进
- Coach 评估的对话窗口与触发时机详见 §4.7.3
...
`project-context.md` 中存在一类**AI 隔离块**——以 `> **AI 行为规则（CRITICAL，所有 AI 必须遵守）— <子主题>**` 开头的 blockquote 块。该类块承载的是**给 iLink bash 脚本读取的工具配置**（如 Issue System 集成的 URL、项目名），不是项目知识。

所有 AI Agent 在读取 `project-context.md` 时 MUST 遵守：

- MUST 识别 AI 隔离块（依据其标题中的"— <子主题>"后缀，与文件最顶部的通用 AI 行为规则块区分）
- MUST NOT 将隔离块内的配置项（如 `api_url`、`project_name` 等）作为业务理解上下文、需求假设或技术决策依据
- MUST NOT 修改隔离块内容；用户请求修改时，MUST 提示用户手动编辑或重跑 `/ilink-bootstrap`
- MUST 在涉及外部系统集成功能（如响应 `/ilink-pull` 类命令）时，将隔离块的存在视为"能力开关"——块缺失则功能不可用，块内值为 `<待填写>` 则需提示用户先编辑

文件最顶部不带 `— <子主题>` 后缀的"AI 行为规则"块是通用 AI 行为规则（关于本文件本身的读写策略），不属于 AI 隔离块——AI MUST 读取并遵守。
...
### 8.1.1 feedback.md 与 .snapshots/ 的特殊地位（v1.6.0）

- `<id>-feedback.md` 是 Coach 角色的旁路输出，文件名 SHALL NOT 携带 `master` 后缀，文档内 SHALL NOT 携带 `ILINK-PROTOCOL-METADATA` 印章；它不被任何下游 AI 角色读取，仅供人类复盘
- `.snapshots/` 目录用于 design.master.md 直接编辑差异检测，由 `/ilink-design` 写入快照，由 `/ilink-approve` 比对差异；MUST 加入 `.gitignore`，SHALL NOT 提交到 git
- 快照命名格式：`design.master.<YYYYMMDD-HHMMSS>.md`，时间戳通过 `TZ=Asia/Shanghai date +%Y%m%d-%H%M%S` 获取
- Coach 比对时按文件名排序取最新快照

### 8.1.2 `<id>-usage.md` Usage 追踪协议（v1.6.0）

**文件性质**：Per-Story 资源消耗观测文件。**纯观测，不参与契约链**——文件名不带 `master` 后缀，文档内 SHALL NOT 携带 `ILINK-PROTOCOL-METADATA` 印章，缺失/缺值 SHALL NOT 阻塞任何角色推进或 Status 流转。

**写入者**：仅两个命令——`/ilink-init`（创建文件 + 写 baseline 行）、`/ilink-qa`（追加 review 行 + 算 delta）。中间角色 PM/Designer/Coder MUST NOT 读写本文件。

**文件位置**：`iLink-doc/<story-id>/<story-id>-usage.md`
...
|------|--------|------------|-----------|
| init | <ISO8601 +08:00> | <数字> | <平台单位> |
| review-1 | <ISO8601 +08:00> | <数字> | <平台单位> |
| review-2 | <ISO8601 +08:00> | <数字> | <平台单位> |

**Latest Delta**: <review_最后一行.value - init.value> (<Usage_Unit>)

> 说明：用户在 init 和每次 qa 执行前，查询本平台 /usage 或 /status 显示的"已使用"数值并传入命令。中间阶段不追踪。回流多次 qa 时按 review-1, review-2... 递增编号，Latest Delta 始终基于最后一次 review。
```

**Usage_Unit 公约**（命令文件硬编码，开发不输入）：

| 平台 | Usage_Unit 取值 | 用户去哪查 |
|------|----------------|-----------|
...
1. 命令第二参数 `<usage-value>` MUST 为非空数字（整数或小数皆可）
2. 缺失第二参数 → 命令拒绝执行，返回平台定制错误文案（例如 Qoder："请先 /usage 查看'已使用 credits'，再以 `/ilink-init <story> <数字>` 重试。无法查询时输入 0。"）
3. 允许传入 `0`，语义为"用户故意跳过查询"——文件正常写入，Latest Delta 字段标注 `(含 0 值, 不可信)`
4. 单位字段由命令文件硬编码 Usage_Unit，**不接受用户输入单位**

**回流行为**：`/ilink-qa` 因 FAIL_BACK_TO_CODER 多次执行时，每次追加新行（review-1, review-2, ...）；Latest Delta 始终用最后一行减 init。中间行不删除，提供"每轮回流烧多少"的可见性。

**跨 reset 边界判定**：若新追加行的 Usage_Value 小于上一行（出现负 delta），说明跨过 Qoder 月账单 / Claude 5h 窗口 / Codex/Gemini CLI 重启边界。Latest Delta 字段 MUST 标注 `(跨 reset, 不可比)` 替代数值。

**与 ilink-status 的关系**：v1.6.0 不修改 ilink-status 行为，本文件仅供人类直接打开查看。未来版本可能扩展 ilink-status 汇总展示。
...
- `iLink-doc/<story>/.snapshots/`（v1.6.0，design 快照仅本地用于 Coach diff，MUST 加入 .gitignore）
- `*.tmp`
...
- bash 脚本依赖：bash、awk、sed、grep、shasum、curl、printf、tr、od（`curl` 与 `od` 主要服务于 `/ilink-pull`——HTTP 调用 + UTF-8 URL encode）
- Windows 用户：所有上述命令在 Git Bash（随 Git for Windows 安装）中默认全部可用，**无需额外安装 Python、jq 等依赖**
- macOS / Linux：上述命令均为系统默认提供
