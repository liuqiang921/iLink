...
读取上游需求定义 `iLink-doc/$ARGUMENTS/$ARGUMENTS-requirement.md`，检查是否包含"关联领域知识"字段且指定了文件路径。如果有，读取该 Domain Knowledge 文件（重点参考 §2 业务实体、§4 接口与集成、§7 设计决策），帮助做出与现有架构一致的技术设计。
