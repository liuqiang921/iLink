...
- 如果有，读取该 Domain Knowledge 文件作为可选参考（主要参考 §6 业务规则）
...
- 如果有，读取该 Domain Knowledge 文件（重点参考 §2 业务实体、§4 接口与集成、§7 设计决策），帮助做出与现有架构一致的技术设计
...
写入 design.master.md 之后，立即执行 shell：
...
如果用户输入不包含两个参数（即缺少 `<usage-value>`），**MUST 拒绝执行**，向用户输出：
...
**Context window used 语义提示**：该值反映**当前 context 占用**而非**累计消耗**，受 Codex CLI 自动压缩 / `/compact` 影响。Latest Delta 适合作为"当前 context 健康度"指标，**不**等同于"本 story 的成本消耗"。
...
1. 读取 `iLink-doc/<story>/<story>-usage.md`
...
4. 在表格末尾（`**Latest Delta**` 行之前）插入一行：`| review-<N+1> | <时间戳> | <usage-value> | codex-context-window-used |`
5. 计算 Latest Delta：`<本次 usage-value> - <init 行的 Usage_Value>`
   - 若结果为正数 → 更新文件末尾 `**Latest Delta**: <delta> (codex-context-window-used)`
   - 若结果为负数 → 更新为 `**Latest Delta**: (跨 reset 边界, 不可比)`（Codex CLI 重启、context 压缩或 session 重置）
   - 若 init 或本次 usage-value 之一为 0 → 更新为 `**Latest Delta**: <delta> (含 0 值, 不可信)`
6. Usage_Unit 字段**硬编码** `codex-context-window-used`，不接受用户输入

> 详细协议见 Root Spec §8.1.2。usage 文件不参与契约链，缺失或写入失败 SHALL NOT 阻塞 Status 推进。

### Usage 追加校验（v1.6.0 MUST）

完成 usage.md 追加后，MUST 向用户回报：
1. 追加的行标识（review-N）
2. Latest Delta 计算结果
3. 若跳过 usage 追加，回报原因（文件缺失/值异常等）
...
### 步骤 1：解析目标 STAGING 文档

按优先级检查：

| 优先级 | 文档 | 处理 |
|-------|------|------|
| 1 | `<story>-review.master.md` STAGING | **跳过 Coach + 不推进 Status**（上游根因，需人工讨论） |
| 2 | `<story>-design.master.md` STAGING | **运行 Coach** + 推进 Status → `PENDING_CODER` |
| 3 | `<story>-pm.master.md` STAGING | **跳过 Coach** + 推进 Status → `PENDING_DESIGNER` |

无 STAGING 文档则告知用户当前无可推进项，结束。

记录 `target_role` 为 `review` / `design` / `pm`，供后续步骤使用。

### 步骤 2：调用 Coach 子流程（仅当 `target_role == "design"` 时执行）

> Root Spec §6.4：Coach 子流程仅在 design.master.md 推进场景启用。PM 与 review 推进场景跳过 Coach，但 MUST 保留 Status 推进逻辑。

若 `target_role != "design"`，跳过步骤 2.1–2.5，直接进入步骤 3，并在最终报告（步骤 4）中说明"本轮非 design 推进，跳过 Coach"。
...
- 列出 `iLink-doc/<story>/.snapshots/design.master.*.md`，按文件名降序取最新
- 执行 `diff -u <最新快照> iLink-doc/<story>/<story>-design.master.md`
- 若快照缺失，记录 `"无法判定直接编辑（缺少 design 快照）"`

#### 2.3 调用 subagent

启动一个 fresh-context 子对话，传入：`coach.soul.md` 全文 + 2.1 对话摘录 + 2.2 diff + 当前时间戳。**SHALL NOT** 传入 master doc、requirement.md、源码。

若当前 Codex 环境实际不支持真正的独立上下文 subagent，MUST 在 feedback.md 该轮反馈段首显式标注 `(未启用 fresh-context, parent 自评)`，便于人类识别该轮反馈可信度下降。
...
#### 2.4 追加写入 feedback.md

- 路径：`iLink-doc/<story>/<story>-feedback.md`
- 文件不存在时新建，**不写入** Metadata 印章
- 追加一个空行，再追加 subagent 返回内容到末尾。SHALL NOT 覆盖历史
- 如果 subagent 响应未以 `## <timestamp> approve 复盘` 标题行开头，Parent AI MUST 在响应前插入该标题行（使用 2.3 prompt 中的同一时间戳）

#### 2.5 异常处理

subagent 失败或返回为空时，**SHALL NOT** 阻塞 Status 推进。在 feedback.md 追加：
```
## <时间戳> approve 复盘
...
仅当步骤 1 命中优先级 2 或 3 时执行：将 STAGING 文档 Metadata 中的 `Status: STAGING` 替换为目标状态值。
...
3. **提取接口与集成（§4）**：grep `@BexMethod` / `@Autowired` / `@DataSource` / `@Value` 等，整理对外功能号、内部依赖、数据库依赖、配置依赖四张表；每行对应可定位的代码（类.方法 / xml 路径 / 配置 key）；提取不到的归入 §10
4. **生成待确认清单**：标出代码里看不出来的"为什么"，登记到 §10
5. **组装完整文档**：按十章标准格式输出

### 输出
- 写入 `iLink-doc/domain/<module>-domain-knowledge.md`
- 十章标准格式：§1 业务定位、§2 业务实体、§3 流程全景、§4 接口与集成、§5 内部机制、§6 业务规则、§7 设计决策、§8 配置参数、§9 故障模式、§10 待确认
...
**SHALL NOT**：
- 自己拼 URL、自己解析 JSON、自己写文件——这些**全部**由 bash 脚本完成
- 自行重试脚本——脚本退出码非 0 时让用户根据 stderr 自行处理
