...
- `<usage-value>`：执行本命令前，用户在 Codex 中执行 `/status` 查看到的 session "tokens used" 数值（仅数字）
...
```
❌ 缺少 usage-value 参数。请先在 Codex 中执行 /status 查看 session "tokens used" 数值，然后以下列格式重试：

  ilink-qa <story-id> <已用 tokens 数字>

例如，/status 显示 tokens used 为 18500，则执行：ilink-qa kcia-1520 18500
...
4. 在表格末尾（`**Latest Delta**` 行之前）插入一行：`| review-<N+1> | <时间戳> | <usage-value> | codex-session-tokens |`
5. 计算 Latest Delta：`<本次 usage-value> - <init 行的 Usage_Value>`
   - 若结果为正数 → 更新文件末尾 `**Latest Delta**: <delta> (codex-session-tokens)`
   - 若结果为负数 → 更新为 `**Latest Delta**: (跨 reset 边界, 不可比)`（Codex CLI 重启或 session 重置）
   - 若 init 或本次 usage-value 之一为 0 → 更新为 `**Latest Delta**: <delta> (含 0 值, 不可信)`
6. Usage_Unit 字段**硬编码** `codex-session-tokens`，不接受用户输入
...
| 优先级 | 文档 | 处理 |
|-------|------|------|
| 1 | `<story>-review.master.md` STAGING | 仅执行 Coach；**不推进 Status**（上游根因，需人工讨论） |
| 2 | `<story>-design.master.md` STAGING | Coach + 推进 Status → `PENDING_CODER` |
| 3 | `<story>-pm.master.md` STAGING | Coach + 推进 Status → `PENDING_DESIGNER` |
...
#### 2.2 计算 design diff（仅当目标为 design）
...
- 目标为 PM 或 review 时，记 `"本段无直接编辑（PM/review 文档不做编辑检测）"`
...
- 将 subagent 返回内容**追加**到末尾（前后加空行），SHALL NOT 覆盖历史
...
输出：
1. ✅ Coach 反馈已追加到 `iLink-doc/<story>/<story>-feedback.md`
...
2. 创建 Story：执行 `bash .codex/commands/ilink-init <story-id> <usage-value>`（v1.6.0：usage-value 必填，传入 `/status` 显示的 session tokens used）
...
4. 创建第一个 Story：bash .codex/commands/ilink-init <story-id> <usage-value>
   （v1.6.0：usage-value 必填。先在 Codex 中 /status 查看 session tokens used 数值后传入）
```
...
| `bash .codex/commands/ilink-init <story> <usage-value>` | 创建 Story 目录和需求模板（v1.6.0：usage-value 必填，传入 `/status` 显示的 session tokens used） |
