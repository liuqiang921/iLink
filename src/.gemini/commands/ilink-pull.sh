#!/usr/bin/env bash
# iLink ilink-pull — Gemini 平台脚本
# 按 story-id 从 Issue System 拉取需求"描述"字段，写入 requirement.md 的"## 功能描述"区块
# 用法: bash .gemini/commands/ilink-pull.sh <story-id>
...
# 定位项目根：脚本位于 <project_root>/.gemini/commands/ilink-pull.sh
...
extract_string_field() {
  ...
  awk -v key="${key}" '
    function find_value_start(json, key, start_idx,    pat, idx, scan) {
      ...
      if (idx == 0) return 0
      scan = start_idx + idx - 1 + length(pat)
