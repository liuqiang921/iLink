...
>
> 该数值仅写入 `<story>-usage.md` 用于事后观察，不参与契约链。无法查询时允许传 `0`，语义为"故意跳过"，对应 delta 标注为"不可信"。详见 §3.8。
...
**执行前**：先在 CLI 中执行平台原生 usage 命令查看"已用"数字（详见 §3.8）。
...
### 3.2 /ilink-pull — 从 Issue 系统拉取需求描述（v1.7.0 新增，可选）

```bash
/ilink-pull kcia-1520
```

**用途**：当你的需求来自外部 Issue 系统（类 Jira，如金证内网 KDOP）时，可一键把 Issue 上的"描述"字段拉到本地 requirement.md 的"## 功能描述"区块，省去手动复制粘贴。

**前置条件**：

1. `project-context.md` 顶部已有"Issue System 集成"AI 隔离块（由 `/ilink-bootstrap` 自动生成）
2. 块内的 `project_name` 已从 `<待填写>` 改为本项目在 Issue 系统中的实际名称
3. 本地 requirement.md 已存在（即先执行过 `/ilink-init`）
4. "## 功能描述"区块仍为空模板（保护人工已编辑的内容）

**执行效果**：

- HTTP GET 调 Issue 系统 → 提取 `data.描述` → 替换"## 功能描述"区块内容
- 区块末尾追加来源行：`> _来源: Issue System (kcia-1520, pulled 2026-05-27T10:30:00+08:00)_`
- 失败时给出明确指引（连接失败 / HTTP 错误 / project_name 未填 / 区块已有内容 等）不破坏现有文件

**典型使用流程**：

```bash
/ilink-init kcia-1520 1200      # 创建本地骨架
/ilink-pull kcia-1520            # 拉取功能描述
# 手动编辑 requirement.md 补全功能范围 / 验收标准 / 约束 / 风险
/ilink-pm kcia-1520              # 进入流水线
```

**不想用的人**：不配置 project_name（保留 `<待填写>`）即可，`/ilink-pull` 会礼貌拒绝，你只需手写"功能描述"内容。完全不影响其他流水线命令。

**底层实现**：纯 bash 脚本（`{.claude,.codex,.qoder,.gemini}/commands/ilink-pull[.sh]`），依赖 `bash + awk + sed + grep + curl + od`——Git Bash on Windows / macOS / Linux 全部默认包含，**零新依赖**。

### 3.3 /ilink-pm — AI 需求分析
...
**执行前**：再次执行平台原生 usage 命令查看"已用"数字（详见 §3.8）。
...
### 3.8 /ilink-init 与 /ilink-qa 的 usage-value 参数（v1.6.0）
...
**解决**：
```bash
# v1.6.0：先在 CLI 中查"已用量"（详见 §3.8），再传入第二个参数
/ilink-init kcia-1520 1200
# 编辑需求定义后再执行
/ilink-pm kcia-1520
```
...
| 章节 | 内容 |
|------|------|
| §1 业务定位 | 定位、职责边界、核心取舍原则 |
| §2 业务实体 | 核心数据结构和关键字段 |
| §3 流程全景 | 业务操作视角 + 线程/调度视角 + 关键路径 |
| §4 接口与集成 | grep 注解机械提取：对外功能号 / 内部依赖 / 数据库依赖 / 配置依赖（四张依赖表） |
| §5 内部机制 | 缓存策略、线程模型、状态机等（复杂模块才写，简单模块省略） |
| §6 业务规则 | 从代码中提取的硬编码规则 |
| §7 设计决策 | 架构选型和权衡 |
| §8 配置参数 | 可调参数清单 |
| §9 故障模式 | 已知的失败路径和恢复策略 |
...
**目的**：给出本模块的**边界全景**——对外暴露哪些接口、依赖哪些内部模块和数据源、用了哪些配置项。这一章不是"AI 推测"，而是 grep 注解可机械证明的代码事实，新人接手、影响面评估、改造前盘点都靠这一章。

**方法**：四个子节都由 grep 模式驱动，提取不到的归入 §10 待确认：

| 子节 | grep 锚点 | 视角 |
|------|----------|------|
| 4.1 对外功能号（消费方） | `@BexMethod(api="...")` | **入站**——本模块暴露的接口，消费方调用本模块时的入口表面 |
| 4.2 内部依赖（提供方） | `@Autowired` / `@Resource` / `bexClient.invoke()` / 跨模块 Bean | **出站**——本模块发起的调用，依赖谁 |
| 4.3 数据库依赖 | `@DataSource` / Mapper XML | 数据源、关键表、双库 Mapper 状态 |
| 4.4 配置依赖 | `@Value` / `application.yml` / `t_dpf_param` | 配置项、配置位置、默认值 |

**典型表**：

| 功能号 (api) | 注册类.方法 | 用途 | 消费方 |
|-------------|------------|------|--------|
| order.submit | OrderApi.submit | 客户下单入口 | 终端 / 报盘网关 |
| order.cancel | OrderApi.cancel | 客户撤单 | 终端 |

| 依赖方 | 调用方式 | 调用入口 | 用途 |
|--------|---------|---------|------|
| 风控模块 | bexClient.invoke | risk.check | 下单前合规校验 |
| 行情模块 | @Autowired | QuoteService.latest | 取最新价撮合 |

**为什么有用**：传统文档需要人类手写"我用了什么"，更新就过时；本章每次重跑 `/ilink-domain` 都从代码现状重新提取，**结构化、可机器对比、不会和代码漂移**。
...
**Domain Knowledge 的审核很重要**：AI 生成的内容里，§4 接口与集成、§2 业务实体、§7 配置参数等代码可证章节通常准确；§1 业务定位、§6 业务规则、§7 设计决策等"为什么"章节可能包含推测，需要资深工程师审核，将 `[待确认]` 项确认为 `[已确认]`。代码变更后建议重跑 `/ilink-domain`，让 §4 四张依赖表跟上最新代码事实。
...
- PM 会参考其中的业务规则（§6）
- Designer 会参考其中的业务实体（§2）、接口与集成（§4）、设计决策（§7）
- 不指定则流水线照常运行，不受影响
...
> **v1.6.0 usage-value**：传入"当前 session/月度已用量"的整数。各平台原生命令：Claude `/usage`、Qoder `/usage`、Codex `/status`、Gemini `/stats`。详见 §3.8。
